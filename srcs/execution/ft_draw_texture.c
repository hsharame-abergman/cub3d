...
/*   Updated: 2025/02/13 23:27:00 by abergman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Функция `ft_tex_init` инициализирует параметры,
	необходимые для отрисовки текстуры на стене в игре или графическом приложении. Она принимает указатель на структуру `t_data`,
...
Далее функция вычисляет координату `texx`,
	которая представляет собой координату текстуры по оси X. Это значение рассчитывается как произведение `wallx`
	и ширины текстуры `WIDTH_TEXTURE`,
	округленное до целого числа. Затем функция корректирует значение `texx` в зависимости от направления игрока.
	Если луч пересек северную или восточную стену (значение `side` равно 0 или 1) и направление игрока по оси X положительное,
...
Наконец, функция вычисляет начальную позицию текстуры `texpos`,
	которая представляет собой координату текстуры по оси Y для верхнего пикселя линии. Это значение рассчитывается
	на основе начальной координаты `start`,
	высоты экрана `HEIGHT`, высоты линии `lineh` и шага `step`.
...
/* Функция `ft_draw_texture` отвечает за отрисовку текстуры на экране для текущей вертикальной полосы,
	...
	Затем вызывается функция `ft_tex_init`, которая инициализирует параметры текстуры.
